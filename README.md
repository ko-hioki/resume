# 職務経歴書
## 基本情報

|key|value|
|---|---|
|氏名|日置 浩司 (Hioki Koji)| 
|生年月日|1989/08|
|居住地|神奈川県鎌倉市|

---

## 職務経歴書
### 株式会社 カヤック (2018/02 ~ 現在) 
カヤックは、ゲームコミュニティから地域通貨サービスのWEBフロントエンドを担当。
長期運用サービスから新規のサービスまで幅広く自社サービスに関わる。

社員ページ: https://www.kayac.com/team/hioki-koji

### RC事業部
#### まちのコイン（プラットフォーム／管理画面）
**期間:** 2019/11 ~ 2025/09  
**役割:** Frontend Engineer / Engineering Manager
**技術:** Next.js, React, TypeScript, SWR, emotion  

地域通貨アプリ「まちのコイン」の管理画面のリプレイス、開発、運用を中心に担当。  
2024〜2025年はディレクション, CS周りも検診しておこなった（フロントエンド 兼任）も担当。

- 管理画面のリニューアル設計・実装
- 保守・機能追加開発
- 開発ディレクション業務
- CS等の業務(半年ほど)
- 技術選定・設計方針の整理

URL: https://coin.machino.co/

#### 自治体イベントサイト
**期間:** 2024/12 ~ 2025/04  
**役割:** Frontend Engineer / Tech Lead  
**技術:** Next.js, TypeScript  

自治体向けイベント掲載サイトのリニューアルを担当。  
これまで年度更新やイベント差し替えの際に3週間程度の作業コストが必要だった更新作業を見直し、運用負荷を削減する設計・実装を行った。  
現在ではキービジュアルの差し替えや細かなファイル更新のみで対応可能な構成としている。

- イベント情報の WordPress 管理対応設計
- 時限式表示ロジックの実装
- 運用コスト削減を目的とした構造改善
- フロントエンド実装のリード

#### SMOUT
**期間:** 2020/11 ~ 2022/12  
**役割:** Frontend Engineer  
**技術:** Nuxt.js (Vue 3), TypeScript, Composition API  

移住マッチングサービスにおいて、レガシーシステムからのモダン化対応や UI の刷新を担当。

- モダンフロントエンドへのリプレイス
- 開発・運用

URL: https://smout.jp

### GC事業部
#### トナメル
**期間:** 2018/02 ~ 2019/11  
**役割:** Frontend Engineer  
**技術:** Typescript, Vue.js, Stylus, GraphQL

大会運営支援ツールのフロント実装を担当。

- 対戦中のチャット機能実装
- トーナメント管理機能対応
- スイスドローの実装

URL: https://tonamel.com/

#### Lobi
**期間:** 2018/02 ~ 2019/11  
**役割:** Frontend Engineer
**技術:** Typescript, jQuery, Vue.js, AngularJS,

コミュニティサービスの保守・運用。  
生配信・ボイスチャット機能などの開発にも関与。

URL: https://web.lobi.co/

### その他
#### まちのスナック
**期間:** 2022/09 ~ 2023  
**役割:** スナックマスター / Engineer  
**技術:** GAS 

地域コミュニティサービスの一環として運営。  
2022/09 にオープン。月1〜2回マスターとして接客に携わる。  
また、LineとFacebookにお知らせを自動で投稿するbot等の作成を行う

URL: https://snack.machino.co/

#### おなりクリーン
**期間:** 2018 ~ 現在  

毎週火曜日の9時から３０分ほど、鎌倉の御成地域のゴミ拾いを行っています。  

SDGsの取り組みとして紹介されています  
https://www.kayac.com/company/sdgs  

#### 育児休暇
**期間:** 2025/09 ~ 2026/02  
**役割:** 育児休暇取得

---
### 株式会社エーピーコミュニケーションズ (2012/04 ~ 2017/12)

- <b>IIJmio</b>
 - 入社後、株式会社インターネットイニシアティブに出向し、
   5年間 MVNOサービス IIJmio フロントエンドの開発/保守/運用を担当
 - サービス全体の機能や音声SIMの申込み、MNP、店舗申し込み、光コラボなどを担当 
  - 技術 
    - JAVA
    - JavaScript です
    - フレームワーク: Struts,Vue.js
---
## アカウント
<p>
<a href="https://github.com/ko-hioki" target="_blank"><img alt="Github" src="https://img.shields.io/badge/ko_hioki-%2312100E.svg?&style=flat-square&logo=Github&logoColor=white" /></a>
<a href="https://twitter.com/hiokky_k" target="_blank"><img alt="Twitter" src="https://img.shields.io/badge/@hiokky_k-%231DA1F2.svg?&style=flat-square&logo=twitter&logoColor=white" /></a>
</p>
